...
void i2c_wakeup()
 {
	...
	ti_lib_i2c_master_init_exp_clk(I2C0_BASE, ti_lib_sys_ctrl_clock_get(),selected_speed);
 }
/*---------------------------------------------------------------------------*/
 static bool
 i2c_status()
...
void i2c_shutdown(void)
{
  ...
  if(selected_pin_sda != I2C_PIN_NOT_SET || selected_pin_scl != I2C_PIN_NOT_SET) {
    i2c_deselect();
  }
...
/*---------------------------------------------------------------------------*/
 bool i2c_write(uint8_t *data, uint8_t len)
 {
   uint32_t i;
   bool success;
...
bool i2c_read(uint8_t *data, uint8_t len)
{
	uint8_t i;
	bool success;
...
bool
i2c_write_single_read_multi2(uint8_t wdata, uint8_t *buf, uint8_t len)
{
	i2c_write_single(wdata);

  return i2c_read(buf,len);
}
/*---------------------------------------------------------------------------*/
void i2c_select(uint32_t new_pin_sda,uint32_t new_pin_scl,uint8_t new_slave_address,
                 bool new_speed,uint32_t new_pin_pull)
 {
	...
	if(new_pin_sda != selected_pin_sda || new_pin_scl != selected_pin_scl ||new_speed != selected_speed) 
	{
	...
	/* Enable and initialize the I2C master module */
	ti_lib_i2c_master_init_exp_clk(I2C0_BASE, ti_lib_sys_ctrl_clock_get(),new_speed);
	}
