...
#include "launchpad/adcin-sensor.h"
#include "sys/timer.h"
...
config(int type, int enable)
{
  ...
  case SENSORS_ACTIVE:
    ...
    ti_lib_rom_ioc_port_configure_set(BOARD_IOID_ALS_OUT, IOC_PORT_GPIO,IOC_STD_OUTPUT);
...
/*---------------------------------------------------------------------------*/
SENSORS_SENSOR(adcin_sensor, ADCIN_SENSOR, value, config, status);
/*---------------------------------------------------------------------------*/
/** @} */
