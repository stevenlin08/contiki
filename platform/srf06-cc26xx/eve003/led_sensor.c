...
typedef enum
{
    ...
    led_type_Bind,     //bind success
    led_type_Action,   //action trigger
    led_type_UnAction,
    led_type_Move,     //device move
    led_type_lowBat,   //device low power
    led_type_IdleBind1,//action bind key and have been bind
    led_type_IdleBind2,//action bind key and have been bind    
    led_type_IdleNoNet,//action bind key and no network
    led_type_SpCood,   //action bind 
    led_type_SpNoCood, //action bind
    led_type_EarseNV
} led_type_t;
...
const struct led_flash_s led_flash_mode[3] = {
    {led_type_NoCoord,led_color_red  ,1},
    {led_type_Bind   ,led_color_green,1},
    {led_type_Bind   ,led_color_green,1},
};
...
uint32_t m_leds_flash(led_type_t p_type, led_color_t p_color, uint16_t p_times)
{
    ...
    else if(p_type == led_type_Bind)
    {
        drv_leds_clr(LED_G1_IDX);

        s_active_leds &= ~((1 << LED_G1_IDX));
        s_blink_leds  &= ~((1 << LED_G1_IDX));

	ctimer_set(&ct, ms(250), m_led_changeHandler, NULL);
	//ctimer_set(&ct, ms(5000), m_led_changeHandler, NULL);
    ...
    else if(p_type == led_type_IdleNoNet)
    {
        drv_leds_clr(LED_R1_IDX);
    ...
    else if (p_type == led_type_EarseNV)
    {   
        drv_leds_clr(LED_R1_IDX);
 	...
 	ctimer_set(&ct, ms(12000), m_led_changeHandler, NULL);			   
        err_code =false;
    }
    else if(p_type == led_type_Action)
    {
        drv_leds_clr(LED_R1_IDX);
        s_active_leds &= ~((1 << LED_R1_IDX));
        s_blink_leds  &= ~((1 << LED_R1_IDX));
	...
	ctimer_set(&ct, ms(1500), m_led_changeHandler, NULL);		
        err_code =false;
    }
    else if(p_type == led_type_UnAction)
    {
        drv_leds_clr(LED_R1_IDX);
        s_active_leds &= ~((1 << LED_R1_IDX));
        s_blink_leds  &= ~((1 << LED_R1_IDX));
	...
	ctimer_set(&ct, ms(100), m_led_changeHandler, NULL);		
        err_code =false;      
    }
...
/*---------------------------------------------------------------------------*/
SENSORS_SENSOR(led_sensor, LED_SENSOR, value_led, config_led, status_led);
